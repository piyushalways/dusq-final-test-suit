...
int fuel_gauge_update(const struct device *charger, bool vbus_connected)
{
	...
	// Convert charging status bitmask to enum
	// 0=Idle, 1=Complete, 2=Trickle, 3=CC, 4=CV
	uint8_t charge_state;
	if (chg_status & NPM1300_CHG_STATUS_COMPLETE_MASK) {
		charge_state = 1; // Complete
	} else if (chg_status & NPM1300_CHG_STATUS_TRICKLE_MASK) {
		charge_state = 2; // Trickle
	} else if (chg_status & NPM1300_CHG_STATUS_CC_MASK) {
		charge_state = 3; // CC (Constant Current)
	} else if (chg_status & NPM1300_CHG_STATUS_CV_MASK) {
		charge_state = 4; // CV (Constant Voltage)
	} else {
		charge_state = 0; // Idle
	}

	// Send fuel gauge data to BLE manager
	struct ble_msg msg = {
		.id = BLE_MSG_ID_FUEL_GAUGE_UPDATE,
		.data.fuel_gauge = {
			.soc = soc,
			.tte = tte,
			.ttf = ttf,
			.chg_status = charge_state
		}
	};
